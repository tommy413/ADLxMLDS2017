...
hw2_special_list = ["klteYv1Uv9A_27_33.avi","5YJaS2Eswg0_22_26.avi","UbmZAe5u5FI_132_141.avi","JntMAcTlOF0_50_70.avi","tJHUH9tpqPg_113_118.avi"]
...
ixs = []
Xtest = []
for ix in test_dict.keys():
	ixs.append(ix)
	Xtest.append(test_dict[ix])

Xtest = np.array(Xtest,dtype="float16")

print("Files loaded.")

video_input = keras.layers.Input([80,4096])
#video_feature = keras.layers.Dense(2048, activation='relu')(video_input)
...
attention = keras.layers.Dense(80, activation='softmax')(attention)
...
hidden = keras.layers.Permute((2,1))(hidden)
hidden = keras.layers.Dense(nb_words + 1, activation='relu')(hidden)

decoder_input = keras.layers.Input(shape=(None, nb_words + 1))
...
decoder_dense = keras.layers.Dense(nb_words + 1, activation='softmax')(decoder_output)

result = keras.layers.Multiply()([decoder_dense, hidden])
...
for i in range(0,Xtest.shape[0]):
	#print(i)
	ans = decoder_str(model,Xtest[i])
	#print(ans)
	if ixs[i] in hw2_special_list:
